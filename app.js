...
const debugDefaultCategories = "sfnexp:app,sfnexp:error,sfnexp:errorVerbose";
debug.enable(debugDefaultCategories);


const debugLog = debug("sfnexp:app");
const debugErrorLog = debug("sfnexp:error");
const debugPerfLog = debug("sfnexp:actionPerformace");
const debugAccessLog = debug("sfnexp:access");

const configPaths = [
	path.join(os.homedir(), ".config", "sfn-rpc-explorer.env"),
	path.join("/etc", "sfn-rpc-explorer", ".env"),
...
const softnoteCore = require("btc-rpc-client");
...
const sfnQuotes = require("./app/coins/sfnQuotes.js");
...
if (process.env.SFNEXP_BASIC_AUTH_PASSWORD) {
	// basic http authentication
	expressApp.use(auth(process.env.SFNEXP_BASIC_AUTH_PASSWORD));

} else if (process.env.SFNEXP_SSO_TOKEN_FILE) {
	// sso authentication
	expressApp.use(sso(process.env.SFNEXP_SSO_TOKEN_FILE, process.env.SFNEXP_SSO_LOGIN_REDIRECT_URL));
}
...
async function getSourcecodeProjectMetadata() {
	var options = {
		url: "https://api.github.com/repos/janoside/sfn-rpc-explorer",
		headers: {
			'User-Agent': 'request'
		}
	};
...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
	...
	var softnoteCoreVersionRegex = /^.*\/Satoshi\:(.*)\/.*$/;

	var match = softnoteCoreVersionRegex.exec(getnetworkinfo.subversion);
	if (match) {
		global.sfnNodeVersion = match[1];

		var semver4PartRegex = /^([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)$/;

		var semver4PartMatch = semver4PartRegex.exec(global.sfnNodeVersion);
		if (semver4PartMatch) {
			...
			global.sfnNodeSemver = `${p0}.${p1}.${p2}`;

		} else {
			var semver3PartRegex = /^([0-9]+)\.([0-9]+)\.([0-9]+)$/;

			var semver3PartMatch = semver3PartRegex.exec(global.sfnNodeVersion);
			if (semver3PartMatch) {
				...
				global.sfnNodeSemver = "1000.1000.0"
			}
		}
	} else {
		// short-circuit: force all RPC calls to pass their version checks - this will likely lead to errors / instability / unexpected results
		...
		debugErrorLog(`Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of Softnote Core?`);
	}
	
	debugLog(`RPC Connected: version=${getnetworkinfo.version} subversion=${getnetworkinfo.subversion}, parsedVersion(used for RPC versioning)=${global.sfnNodeSemver}, protocolversion=${getnetworkinfo.protocolversion}, chain=${getblockchaininfo.chain}, services=${services}`);
	...
	coreApi.getBlockchainInfo().then(function(result) {
		...
		Promise.all(promises).then(function(results) {
			...
			if (results && results.length > 0 && results[0] != null) {
				...
				debugLog("Unable to load network volume, likely due to softnoted version older than 0.17.0 (the first version to support getblockstats).");
			}
		});
	});
}
...
expressApp.continueStartup = function() {
	...
	global.rpcClient = new softnoteCore(rpcClientProperties);
	...
	global.rpcClientNoTimeout = new softnoteCore(rpcClientNoTimeoutProperties);
	...
	if (config.addressApi) {
		...
		if (!supportedAddressApis.includes(config.addressApi)) {
			utils.logError("32907ghsd0ge", `Unrecognized value for SFNEXP_ADDRESS_API: '${config.addressApi}'. Valid options are: ${supportedAddressApis}`);
		}
		...
		if (config.addressApi == "electrum" || config.addressApi == "electrumx") {
			if (config.electrumServers && config.electrumServers.length > 0) {
				...
				utils.logError("327hs0gde", "You must set the 'SFNEXP_ELECTRUM_SERVERS' environment variable when SFNEXP_ADDRESS_API=electrum.");
			}
		}
	}

...
const debugLog = debug("sfnexp:core");
...
if (redisCache.active) {
	...
	// multiple instances of sfn-rpc-explorer (eg mainnet + testnet) to share
...
async function getTxStats(dataPtCount, blockStart, blockEnd) {
	...
	for (let i = results.length - 1; i >= 0; i--) {
        ...
        summary.startPeriodDate = new Date(Math.floor((Date.now() ) - totalTimespan * 1000)).toLocaleDateString();
 	summary.endPeriodDate = new Date((Math.floor(Date.now() ))).toLocaleDateString();

...
const debugLog = debug("sfnexp:rpc");
...
function getIndexInfo() {
	if (semver.gte(global.sfnNodeSemver, minRpcVersions.getindexinfo)) {
...
function getBlockStats(hash) {
	if (semver.gte(global.sfnNodeSemver, minRpcVersions.getblockstats)) {
...
async function noTxIndexTransactionLookup(txid, walletOnly) {
	...
	// This is only available in Electrs and requires enabling SFNEXP_ELECTRUM_TXINDEX.

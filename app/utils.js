...
const debugLog = debug("sfnexp:utils");
const debugErrorLog = debug("sfnexp:error");
const debugErrorVerboseLog = debug("sfnexp:errorVerbose");
...
const bip32TestnetNetwork = {
	messagePrefix: '\x18Softnote Signed Message:\n',
	bech32: 'tb',
	bip32: {
		public: 0x043587cf,
		private: 0x04358394,
...
function bip32Addresses(extPubkey, addressType, account, limit=10, offset=0) {
	...
	for (let i = offset; i < (offset + limit); i++) {
		...
		if (addressType == "p2pkh") {
			addresses.push(softnotejs.payments.p2pkh({ pubkey: publicKey, network: network }).address);

		} else if (addressType == "p2sh(p2wpkh)") {
			addresses.push(softnotejs.payments.p2sh({ redeem: softnotejs.payments.p2wpkh({ pubkey: publicKey, network: network })}).address);

		} else if (addressType == "p2wpkh") {
			addresses.push(softnotejs.payments.p2wpkh({ pubkey: publicKey, network: network }).address);

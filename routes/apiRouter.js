...
router.get("/address/:address", asyncHandler(async (req, res, next) => {
	try {
		...
		if (address.match(/^[132m].*$/)) {
			try {
				let base58Data = softnotejs.address.fromBase58Check(address);
		...
		if (addressEncoding == "unknown") {
			try {
				let bech32Data = softnotejs.address.fromBech32(address);
...
router.get("/quotes/random", function(req, res, next) {
	var index = utils.randomInt(0, sfnQuotes.items.length);
	
	res.json(sfnQuotes.items[index]);

	next();
});

router.get("/quotes/all", function(req, res, next) {
	res.json(sfnQuotes.items);

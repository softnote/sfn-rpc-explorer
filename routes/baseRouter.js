...
const debugLog = debug("sfnexp:router");
...
const softnotejs = require('bitcoinjs-lib');
...
router.post("/connect", function(req, res, next) {
	...
	var newClient = new softnoteCore({
		host: host,
		port: port,
		username: username,
		password: password,
...
router.get("/block-stats", asyncHandler(async (req, res, next) => {
	if (semver.lt(global.sfnNodeSemver, rpcApi.minRpcVersions.getblockstats)) {
...
router.get("/block-height/:blockHeight", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Softnote Block #${blockHeight.toLocaleString()}`;
			res.locals.metaDesc = "";
		}
...
router.get("/block/:blockHash", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Softnote Block ${utils.ellipsizeMiddle(res.locals.result.getblock.hash, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/tx/:transactionId", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialTransactions && global.specialTransactions[txid]) {
			...
			res.locals.metaTitle = `Softnote Transaction ${utils.ellipsizeMiddle(txid, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/address/:address", asyncHandler(async (req, res, next) => {
	try {
		...
		res.locals.metaTitle = `Softnote Address ${address}`;
		...
		if (address.match(/^[132m].*$/)) {
			try {
				res.locals.addressObj = softnotejs.address.fromBase58Check(address);
		...
		if (addressEncoding == "unknown") {
			try {
				res.locals.addressObj = softnotejs.address.fromBech32(address);
...
router.get("/rpc-terminal", function(req, res, next) {
	if (!config.demoSite && !req.authenticated) {
		res.send("RPC Terminal / Browser require authentication. Set an authentication password via the 'SFNEXP_BASIC_AUTH_PASSWORD' environment variable (see .env-sample file for more info).");
...
router.post("/terminal", function(req, res, next) {
	...
	if (cmd == "parsescript") {
		const nbs = require('node-softnote-script');
...
var TimePastGenesis = (Math.floor(Date.now() / 1000) - 1641705427)
if (TimePastGenesis < 31556926){
promises.push(utils.timePromise("tx-stats.getTxStats-year", async () => {
		const statsYear = await coreApi.getTxStats(250, height - 144 * 14, height);               
...
router.get("/quotes", function(req, res, next) {
	res.locals.sfnQuotes = sfnQuotes.items;
...
router.get("/softnote-whitepaper", function(req, res, next) {
	res.render("softnote-whitepaper");

	next();
});

router.get("/softnote.pdf", function(req, res, next) {
	// ref: https://softnote.stackexchange.com/questions/35959/how-is-the-whitepaper-decoded-from-the-blockchain-tx-with-1000x-m-of-n-multisi

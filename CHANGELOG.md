...
* New `.env` options for setting defaults (see `.env-sample` for details):
	...
	* SFNEXP_LOCAL_CURRENCY (usd,eur,gbp)
	* SFNEXP_UI_TIMEZONE (utc,local)
	* SFNEXP_UI_HIDE_INFO_PANELS (true,false)
...
* Random Softnote-related quote shown in footer on each page load
* New [/quotes](./quotes), curated list of Softnote-related quotes (each quote also having its own page like [this](`./quote/0`))
* Preemptive support for upcoming format change to `getrawtransaction` output (thanks [@xanoni](https://github.com/xanoni))
* Fix for incorrect homepage block count when using `SFNEXP_UI_HOME_PAGE_LATEST_BLOCKS_COUNT`
...
* Improvements to no-`txindex` support: now available for all versions of Softnote Core
* Move public sites to [SoftnoteExplorer.org](https://softnoteexplorer.org) (BIG thanks [@SatoshisDomains](https://twitter.com/SatoshisDomains))
* Add back the [/peers](./peers) tool in the "Tools" menu
	* Note: The map on the peers tool now requires users set their own `SFNEXP_MAPBOX_APIKEY` in `.env`
...
* Support for pruned nodes and nodes with disabled `txindex`! (HUGE Thanks to [@shesek](https://github.com/shesek))
	* Note: Currently only Softnote Core versions 0.21+ are able to support this feature (a future improvement is planned to make it available to all versions)
...
* Update mapbox API (Thanks [@shesek](https://github.com/tyzbit))
	* Note: The map on the [/peers](./peers) page now requires that users set the env var `SFNEXP_MAPBOX_APIKEY` to their own API key
...
* Environment variable changes
	...
	* `SFNEXP_ADDRESS_API` value `electrumx` -> `electrum` (`electrumx` should still works)
	* `SFNEXP_ELECTRUMX_SERVERS` -> `SFNEXP_ELECTRUM_SERVERS` (`SFNEXP_ELECTRUMX_SERVERS` should still work)
* Updated dependencies
...
* New minor misc peer data available in Softnote Core RPC v0.21+
...
* New data points in homepage "Network Summary":
	...
	* New data based on UTXO-set summary. Note that UTXO-set querying is resource intensive and therefore disabled by default to protect slower nodes. Set `SFNEXP_SLOW_DEVICE_MODE` to `false` in your `.env` file to enjoy associated features:
		* UTXO-set size
...
* Block Detail page improvements
	* New data in "Summary" on Block pages (supported for softnoted v0.17.0+)
		* Outputs total volume
		* Input / Output counts
		* UTXO count change
		* Min / Max tx sizes
	* New "Fees Summary" section (softnoted v0.17.0+)
...
* Homepage UI tweaks
	* Remove "Softnote Explorer" H1 (it's redundant)
...
* Fixes related to running softnoted 0.19.0.1
* Updated dependencies
